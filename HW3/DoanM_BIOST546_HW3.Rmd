...
```
The `wdbc` data set has a sample size of $n$ = `r nrow(wdbc_set)` and $p$ = `r ncol(wdbc_set[, -1])` predictors. There are `n wdbc_summary[1, 2]` observations in the benign class and `n wdbc_summary[2, 2]` observations in the malignant class (as shown in **Table 1**).
